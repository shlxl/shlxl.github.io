...
title: 站点内容脚手架与发布全指南（含博客与作品集）
date: 2025/09/14 10:20:00
description: 以 new:post 脚手架为核心，整合博客新增、作品集维护、预览与推送规范，一文搞定。
tags: [脚手架, 自动化, 博客, 作品集]
categories: [文档]
cover: /images/scaffold-guide-cover.svg
top: 1
...
本文基于内置脚手架命令，整合「新增博客文章」「新增作品集内容」与「预览发布」的完整流程，并已全面采用“绝对时间”显示与 +8 时区配置，确保时间一致可控。

提示：本站已切换为“绝对时间”显示（yyyy/MM/dd HH:mm），并统一使用 +8 时区；所有示例均采用斜杠日期格式以避免解析歧义。
...
规范建议：
- 文件名/slug：小写-中划线；非 ASCII 会被移除
- 日期：`yyyy/MM/dd HH:mm:ss`（用于排序；脚手架已按 Asia/Shanghai 生成）
- 置顶：`top: 1/2/...`（数字越小越靠前）
...
- 图片 404：确保放 `docs/public/` 并用以 `/` 开头的绝对路径引用
- 排序异常：检查 `date` 格式；置顶用 `top: 1/2/...`
- 时间显示：已统一为绝对时间（`yyyy/MM/dd HH:mm`），避免相对时间带来的时区误差
