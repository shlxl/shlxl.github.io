...
if(!slug){ console.error('用法: node scripts/post-archive.mjs <slug>'); process.exit(1); }

const file = findMdBySlug(BLOG_DIR, slug);
if(!file){ console.error(`[archive] 未找到 slug=${slug}`); process.exit(2); }

let txt = fs.readFileSync(file,'utf8');
const m = /^---\s*([\s\S]*?)\s*---/m.exec(txt);
if(!m){ console.error(`[archive] 文件缺少 frontmatter: ${file}`); process.exit(3); }
let fm = m[1];
